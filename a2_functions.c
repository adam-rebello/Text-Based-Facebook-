/********* definitions.c ********
    Student Name 	= Adam Rebello
    Student Number	= 101258440
*/
...
user_t *add_user(user_t *users, const char *username, const char *password)
{
   ...
   user->friends = NULL;
   user->posts = NULL;
   if (users == NULL || strcmp(user->username, users->username) < 0) //if list is empty or if new node data < head node data
   {
      user->next = users;
      return user;
   }
...
void add_friend(user_t *user, const char *friend)
{
   ...
   if (user->friends == NULL || strcmp(friend, user->friends->username) < 0) //if list is empty or new node data is < head node data
   {
...
_Bool delete_friend(user_t *user, char *friend_name)
{
   ...
   if (current != NULL) // check if position is valid
   {
      if (previous != NULL) // update the next pointer of the previous node
      {
